...
export default function DashboardPage() {
  ...
  const invitationHref = event.invitation_url || (guests[0] ? `/evento/${event.slug}/${guests[0].code}` : null);
  return <section className="dashboard">
    <header className="page-header"><div><span className="page-eyebrow">Resumen del evento</span><h1>{event.name}</h1><p>{day} {month} {year} · Plan {event.plan}</p></div>{invitationHref ? <a className="button button--dark" href={invitationHref} target="_blank" rel="noreferrer"><ExternalLink size={17} /> Ver invitación</a> : <Link className="button button--dark" to="/panel/invitados"><Users size={17} /> Agregar primer invitado</Link>}</header>
