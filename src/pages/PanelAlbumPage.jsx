...
function Photo({ photo, onHide }) {
  ...
  return <article>{isVideoMedia(photo) ? <video src={source} controls playsInline preload="metadata" aria-label={`Video de ${photo.author}`} /> : <img src={source} alt={`Fotografía de ${photo.author}`} loading="lazy" decoding="async" />}<div><span><strong>{photo.author}</strong><small>{new Intl.DateTimeFormat("es-MX", { dateStyle: "medium" }).format(new Date(photo.createdAt))}</small></span><button type="button" onClick={() => onHide(photo)} aria-label="Ocultar recuerdo"><EyeOff /></button></div></article>;
}
