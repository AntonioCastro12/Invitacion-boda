...
import { hideOwnerAlbumPhoto, isVideoMedia, listOwnerAlbumPhotos } from "../services/albumService";
...
function Photo({ photo, onHide }) {
  ...
  return <article>{isVideoMedia(photo) ? <video src={source} controls playsInline preload="metadata" aria-label={`Video de ${photo.author}`} /> : <img src={source} alt={`Fotografía de ${photo.author}`} loading="lazy" decoding="async" />}<div><span><strong>{photo.author}</strong><small>{new Intl.DateTimeFormat("es-MX", { dateStyle: "medium" }).format(new Date(photo.createdAt))}</small></span>{!photo.sample && <button type="button" onClick={() => onHide(photo)} aria-label="Ocultar recuerdo"><EyeOff /></button>}</div></article>;
}
...
export default function PanelAlbumPage() {
  ...
  return <section><header className="page-header"><div><span className="page-eyebrow">Moderación privada</span><h1>Álbum digital</h1><p>Las fotografías y videos pertenecen exclusivamente a {event.name}.</p></div><Link className="button button--dark" to={`/album/${event.slug}/A7X92`} target="_blank"><Images size={17} /> Ver álbum</Link></header><div className="privacy-callout"><ShieldCheck /><div><strong>Álbum aislado por evento</strong><p>Los invitados de otras celebraciones no pueden consultar ni publicar en este álbum. Las URLs reales caducan automáticamente.</p></div></div>{notice && <div className="album-notice">{notice}</div>}<div className="panel-album-grid">{photos.map((photo) => <Photo key={photo.id} photo={photo} onHide={hide} />)}</div></section>;
}
