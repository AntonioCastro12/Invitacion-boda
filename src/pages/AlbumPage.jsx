...
export default function AlbumPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try { const result = await listAlbumPhotos(invitation.event, invitation.guest, 0); setPhotos(result.photos); setPage(0); setHasMore(Boolean(result.hasMore)); }
  ...
  async function loadMore() {
    if (!invitation || !hasMore) return;
    const nextPage = page + 1;
    try { const result = await listAlbumPhotos(invitation.event, invitation.guest, nextPage); setPhotos((current) => [...current, ...result.photos]); setPage(nextPage); setHasMore(Boolean(result.hasMore)); }
    catch (reason) { setNotice(reason.message || "No fue posible cargar más fotografías."); }
  }
  ...
  const albumCover = event.template_config?.album_cover || event.template_config?.gallery?.[0];
  const albumHeroStyle = albumCover
