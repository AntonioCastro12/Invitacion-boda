...
export async function listAlbumPhotos(event, guest, page = 0) {
  if (!isSupabaseConfigured) {
    ...
    return {
      shared: false,
      photos: [...local.map((photo) => ({ ...photo, author: photo.author, createdAt: photo.createdAt, local: true })), ...samplesFor(event)]
    };
  }

  const { data, error } = await requireSupabase().functions.invoke("album-access", { body: { eventSlug: event.slug, guestCode: guest.code, page } });
  if (error) throw error;
  const uploaded = (Array.isArray(data?.photos) ? data.photos : []).map((photo) => ({
    id: photo.id,
  ...
  return { shared: true, hasMore: Boolean(data?.hasMore), photos: [...uploaded, ...(page === 0 ? samplesFor(event) : [])] };
}

export async function listOwnerAlbumPhotos(event) {
  if (!isSupabaseConfigured) return listAlbumPhotos(event, { code: "A7X92" });
  const client = requireSupabase();
  const { data, error } = await client.from("album_photos").select("*").eq("event_id", event.id).eq("status", "visible").order("created_at", { ascending: false }).limit(200);
  if (error) throw error;
  if (!data.length) return { shared: true, photos: [] };
  const { data: signed, error: signedError } = await client.storage.from("event-albums").createSignedUrls(data.map((photo) => photo.storage_path), 600);
  if (signedError) throw signedError;
  return { shared: true, photos: data.map((photo, index) => ({ ...photo, author: photo.uploader_name, createdAt: photo.created_at, url: signed[index]?.signedUrl, shared: true })) };
}

export async function hideOwnerAlbumPhoto(photo) {
  if (photo.local) return deleteLocalPhoto(photo.id);
  const { error } = await requireSupabase().from("album_photos").update({ status: "hidden" }).eq("id", photo.id);
  if (error) throw error;
}
...
export async function uploadAlbumPhotos(event, guest, files) {
  ...
  for (const file of images) {
    ...
    const { data: authorization, error: authorizationError } = await client.functions.invoke("album-access", { body: { action: "upload-url", eventSlug: event.slug, guestCode: guest.code, fileName: file.name, mimeType: file.type, size: file.size } });
    if (authorizationError) throw authorizationError;
    const { error: uploadError } = await client.storage.from("event-albums").uploadToSignedUrl(authorization.path, authorization.token, file, { cacheControl: "3600", contentType: file.type });
