...
export default function WelcomeScreen({ event, onOpen }) {
  ...
  const advance = () =>
    stage < LAST_STAGE ? setStage((current) => current + 1) : onOpen();
  const labels = [
    "Abrir el sobre",
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Presentación de la invitación"
      exit={
        reduceMotion
          ? undefined
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {stage === 0 && (
            ...
            >
              <span
                className="screen-envelope screen-envelope--closed"
                aria-hidden="true"
              >
                ...
                <span className="screen-envelope__recipient">
                  ...
                  <strong>
                    {names.first} <i>&amp;</i> {names.second}
                  </strong>
                </span>
                <span className="screen-envelope__date-mark">
                  {displayDate}
                </span>
                <motion.span
                  ...
                  animate={
                    reduceMotion
                      ? undefined
                      : { scale: [1, 1.055, 1], rotate: [0, -1.2, 0] }
                  }
          ...
          {stage === 1 && (
            ...
            >
              <span
                className="screen-envelope screen-envelope--open"
                aria-hidden="true"
              >
                <span className="screen-envelope__paper-grain" />
                <span className="screen-envelope__flap" />
                <span className="screen-envelope__letter">
                  <span className="screen-envelope__letter-monogram">
                    {initials[0]} <i>&amp;</i> {initials[1]}
                  </span>
                  <span className="screen-envelope__letter-copy">
                    Tenemos algo especial que contarte…
                  </span>
                  <span className="screen-envelope__letter-names">
                    {event.name}
                  </span>
              ...
              <span className="welcome__instruction welcome__instruction--after-open">
                Toca para continuar
              </span>
            </motion.span>
          )}
          {stage === 2 && (
            ...
            >
              <span className="welcome__ornament">
                <i />
              </span>
              <span className="welcome__announcement">¡Nos casamos!</span>
              <span className="welcome__names">
                {names.first} <i>&amp;</i> {names.second}
              </span>
              <Heart className="welcome__heart" size={18} strokeWidth={1.1} />
              <span className="welcome__instruction">
                Toca para descubrir la fecha
              </span>
            </motion.span>
          )}
          {stage === 3 && (
            ...
            >
              ...
              <span className="welcome__waiting">Te esperamos</span>
              <span className="welcome__date">{displayDate}</span>
              <motion.span
                ...
                animate={
                  reduceMotion
                    ...
                    : {
                        boxShadow: [
                          "0 12px 28px rgba(78,87,66,.18)",
                          "0 17px 38px rgba(78,87,66,.3)",
                          "0 12px 28px rgba(78,87,66,.18)",
                        ],
                      }
                }
